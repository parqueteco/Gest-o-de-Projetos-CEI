...
async function run() {
  const { data, error } = await supabase.from('acoes').select('*').limit(1);
  if (data && data.length > 0) {
     console.log('Action found:', data[0].idacao);
     // Let's check if we can delete a dummy row instead. We already deleted 'test' pilar.
  }
}
run();
