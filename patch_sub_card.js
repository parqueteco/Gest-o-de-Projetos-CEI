...
const startTag = '                                              <div className="flex items-start gap-3 group p-3 rounded bg-slate-950/40 hover:bg-slate-900/80 transition-colors border border-slate-800/80 hover:border-slate-700">';
const endTag = '                                              </div>';
...
let depth = 1;
while (depth > 0 && currentIndex < code.length) {
    const nextDivOpen = code.indexOf('<div', currentIndex);
    const nextDivClose = code.indexOf('</div', currentIndex);
...
if (depth !== 0) {
    console.error("Could not find matching end tag!");
    process.exit(1);
}

const target = code.substring(startIndex, currentIndex);
...
code = code.substring(0, startIndex) + replacement + code.substring(currentIndex);
